...
void construct_track_nbs(ifstream& infile, int t[][4096], int* l){
	...
	//该函数从给定文件指针（已定位到记录音轨的部分，详见find_track_begin函数），读取音符事件到track[32][4096]数组，将音轨长度（最后一个音的时间）记录于数组l

	int h_jump, v_jump, h_pos = -1, v_pos = -1, p, unused;
	// h_jump, v_jump:（到下一个音的）水平时间跳转距离，垂直音轨跳转距离
	// h_pos, v_pos: 当前时间位置，当前音轨位置
	// p, unused: 音高，音符的其他未使用属性（我只关注单个变量，如有需要可合并为结构体）

	bool tick_changed = false;						//记录是否发生列（时间）跳转，用于判断音轨结束
	do {											//读到水平0，垂直0代表列结束；水平0，垂直0，水平0代表文件结束
		type_read(infile, Short, &h_jump);
		if (h_jump == 0)
			if (tick_changed) return;	//文件结束
		h_pos += h_jump;
...
int main(int argc, char* argv[]) {
	...
	for (int a = argc; a > 1; a--) {
		...
		if (file_type == "nbp") {
			...
			string nb; char nb_in;//从.nbp文件读取的音轨
